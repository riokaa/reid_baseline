...
class CUHK03(BaseImageDataset):
    ...
    def _check_before_run(self):
        ...
        if not osp.exists(self.split_new_det_mat_path):
            raise RuntimeError(
                "'{}' is not available".format(self.split_new_det_mat_path)
            )
        if not osp.exists(self.split_new_lab_mat_path):
            raise RuntimeError(
                "'{}' is not available".format(self.split_new_lab_mat_path)
            )
    ...
    def _preprocess(self):
        ...
        def _extract_img(name):
            ...
            meta_data = []
            imgs_dir = (
                self.imgs_detected_dir if name == "detected" else self.imgs_labeled_dir
            )
